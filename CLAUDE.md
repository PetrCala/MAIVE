...
1. **Input Validation** (`normalize_maive_options`): Validates and normalizes all input parameters and resolves column mappings (`estimate`, `se`, `n`, optional `study_id`)
...
- `beta_standard`, `SE_standard`: Conventional (non-IV) estimate and standard error from the same fit
